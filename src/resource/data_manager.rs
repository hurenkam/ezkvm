...
static RESOURCE_MANAGER: Lazy<Arc<Mutex<DataManager>>> = Lazy::new(||Arc::new(Mutex::new(DataManager::new("/etc/ezkvm/resource"))));
...
impl DataManager {
    fn new(path: &str) -> Self {
        ...
        info!("ResourceManager::new() locked_resources:\n{:?}",locked_resources);
    ...
    pub fn claim_resource(&mut self, pool: String) -> Result<String, EzkvmError> {
        ...
        if let Some(resource_pool) = self.resources.get(&pool) {
            ...
            self.locked_resources.insert(id.clone(),self.current_lock.get_name());
            return Ok(id);
        }

        debug!("DataManager::claim_resource() Resource '{}' not available.",pool);
...
fn load_resource_pools() -> Result<HashMap<String, ResourcePool>, EzkvmError> {
    ...
    let files = fs::read_dir("/etc/ezkvm/resources/").map_err(|_|EzkvmError::OpenError { file: "/etc/ezkvm/resources/".to_string() })?;
    for file in files {
...
fn load_machine_locks() -> Result<HashMap<String,Lock>, EzkvmError> {
    ...
    let files = fs::read_dir("/var/ezkvm/lock/").map_err(|_|EzkvmError::OpenError { file: "/var/ezkvm/lock/".to_string() })?;
    for file in files {
        if let Ok(entry) = file {
            if let Ok(file_name) = entry.file_name().into_string() {
                if let Some(base_name) = file_name.strip_suffix(".yaml") {
...
fn find_locked_resources(resources: &HashMap<String,ResourcePool>, locks: &HashMap<String,Lock>) -> HashMap<String,String> {
