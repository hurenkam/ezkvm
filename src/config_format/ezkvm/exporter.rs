...
impl RuntimeModelExporter for EzkvmExporter {
    ...
    fn export(&self, runtime: &RuntimeModel, _args: ExportOptions) -> Result<PathBuf, ExportError> {
        let runtime_config = RuntimeConfig::try_from(runtime).map_err(|e| {
            ExportError::ExportFailed(format!("Failed to convert runtime model: {}", e))
        })?;
        ...
        Exporter::export(
            ...
            ExportOptions::Ezkvm {
                host: String::new(),
                vm: Some(path.to_string_lossy().to_string()),
            },
        )
    }
}
