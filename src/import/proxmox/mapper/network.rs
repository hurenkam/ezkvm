...
pub(super) fn map_network(
    ...
    let backend_type = if has_bridge {
        match runtime_target {
            RuntimeTarget::PortableLinux => "bridge",
            RuntimeTarget::ProxmoxParity => "tap",
        }
        .to_string()
    } else {
        "user".to_string()
    ...
    let ifname = if runtime_target == RuntimeTarget::ProxmoxParity {
        network
            .options
            .get("ifname")
            .cloned()
    ...
    let script = if runtime_target == RuntimeTarget::ProxmoxParity {
        network.options.get("script").cloned()
    } else {
        None
    };
    let downscript = if runtime_target == RuntimeTarget::ProxmoxParity {
        network.options.get("downscript").cloned()
    } else {
        None
    };
    let vhost = match backend_type.as_str() {
        "tap" => network
            .options
            .get("vhost")
