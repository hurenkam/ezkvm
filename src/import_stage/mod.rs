...
use crate::vm_spec::model::VirtualMachine;
...
impl ProxmoxConfImportStage {
    ...
    fn parse(request: ImportRequest<'_>) -> Result<CanonicalDocument, ProxmoxConfImportError> {
...
impl ImportStage for ProxmoxConfImportStage {
    ...
    fn import(&self, request: ImportRequest<'_>) -> Result<CanonicalDocument, Self::Error> {
        Self::parse(request).map_err(Into::into)
    }
}
...
fn parse_machine_value(value: &str, source_name: &str) -> Result<Machine, ProxmoxConfImportError> {
...
"#
    ...
    fn proxmox_conf_import_stage_rejects_malformed_source_name() {
        ...
        assert!(matches!(
            err,
            ImportStageError::ProxmoxConf(ProxmoxConfImportError::InvalidSourceName { .. })
        ));
    }
}
