...
static OVMF_SIZE: LazyLock<HashMap<OVMFSize,(u64,u64)>> = LazyLock::new(||HashMap::from([
    // type                 rom  settings
...
impl OVMF {
    ...
    pub fn secure_boot_default() -> Option<bool> { Some(true) }
        ...
        let result = match arch {
            ...
            OVMFArch::Arch64 => {
                ...
                if size == OVMFSize::Size4M {
                    if secure_boot { OVMF64_4M_SECURE_BOOT_ROM } else { OVMF64_4M_BOOT_ROM }
                } else {
                    if secure_boot { OVMF64_2M_SECURE_BOOT_ROM } else { OVMF64_2M_BOOT_ROM }
                }
            }
        };
    ...
    fn get_qemu_args(&self, _index: usize) -> Vec<String> {
        vec![
            ...
            format!(
                ...
                self.boot_rom_file(), self.boot_rom_size()
            ),
            format!(
                "-drive if=pflash,unit=1,id=drive-efidisk0,format=raw{}{}",
                self.settings_file(), self.settings_size()
            ),
        ]
    }
}
...
mod tests {
    ...
    fn test_valid() {
        ...
        assert_eq!(ovmf.get_qemu_args(0), vec![
            ...
            "-drive if=pflash,unit=0,format=raw,readonly=on,file=/usr/share/ezkvm/OVMF_CODE.fd".to_string(),
            "-drive if=pflash,unit=1,id=drive-efidisk0,format=raw,file=the_file,size=131072".to_string()
        ]);
    }
}
