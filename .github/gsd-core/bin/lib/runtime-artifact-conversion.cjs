...
// where it resolved (the agent's synthetic `{"type":"commonjs"}`), there is no
 ...
 *   Global: .github/ → ~/.copilot/, ./.github/ → ./.github/
 *   Local:  .github/ → ./.github/, ./.github/ → ./.github/
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
    ...
    // Handle both `.github/foo` (trailing slash) and bare `.github` forms in
 ...
 * Path mappings depend on install mode:
 *   Global: .github/ → ~/.gemini/antigravity/, ./.github/ → ./.agents/
 *   Local:  .github/ → .agents/, ./.github/ → ./.agents/
...
function convertClaudeCommandToWindsurfWorkflow(content, commandName) {
    ...
    const workflow = `# ${commandName}\n\n${toSingleLine(description || `Run ${commandName}.`)}\n\nRead and execute the GSD command at @.github/gsd-core/commands/gsd/${stem}.md end-to-end. Treat the user's message after /${commandName} as the command arguments.`;
 ...
 * (.github/→.cline/, "Claude Code"→"Cline", etc.).
...
function convertClaudeCommandToClineSkill(content, skillName, _runtime = null, cmdNames = null) {
    ...
    // apply Cline-specific markdown rewrites (.github/→.cline/, etc.).
    const names = cmdNames || readGsdCommandNames();
    const normalizedBody = transformContentToHyphen(body, names);
...
function convertClaudeToCodexMarkdown(content) {
    ...
    converted = converted.replace(/\.\/\.claude\//g, './.codex/');
    // Bare .github without trailing slash (e.g. configDir = .github)
    converted = converted.replace(/\$HOME\/\.claude\b/g, '$HOME/.codex');
    converted = converted.replace(/~\/\.claude\b/g, '~/.codex');
    // Bare/project-relative .github/... references (#2639). Covers strings like
    // "check `.github/skills/`" where there is no ~/, $HOME/, or ./ anchor.
...
function neutralizeAgentReferences(content, instructionFile) {
    ...
    // Replace copilot-instructions.md with runtime-appropriate instruction file
    if (instructionFile) {
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
    ...
    // Replace .github and .github with OpenCode's config location
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
    ...
    convertedContent = convertedContent.replace(/\/gsd-/g, '/gsd-');
    // Replace .github and .github with Kilo's config location
    ...
    convertedContent = replaceRelativePathReference(convertedContent, '.github/skills/', '.kilo/skills/');
    convertedContent = replaceRelativePathReference(convertedContent, '.agents/skills/', '.kilo/skills/');
    convertedContent = replaceRelativePathReference(convertedContent, '.github/agents/', '.kilo/agents/');
 ...
 *
 * Body: preserved verbatim after the qwen branding rewrite (copilot-instructions.md /
 * Claude Code / .github/ literal-substring values — descriptor-driven via
 * runtime.hostBehaviors.brandingRewrites, mirrors the qwen case in
 * _applyRuntimeRewrites). The anchored `.github/` / `.github/` forms
...
function convertClaudeAgentToQwenAgent(content) {
    ...
    let converted = content;
    if (_b['copilot-instructions.md'])
        converted = converted.replace(/CLAUDE\.md/g, _b['copilot-instructions.md']);
    if (_b['Claude Code'])
        converted = converted.replace(/\bClaude Code\b/g, _b['Claude Code']);
    if (_b['.github/'])
        converted = converted.replace(/\.claude\//g, _b['.github/']);
    const { frontmatter, body } = extractFrontmatterAndBody(converted);
    if (!frontmatter)
        return converted;
    const name = extractFrontmatterField(frontmatter, 'name') || 'unknown';
...
function _applyRuntimeRewrites(content, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    ...
    switch (runtime) {
        ...
        // brand VALUES (copilot-instructions.md/Claude Code/.github/ replacements) now read from
        ...
        case 'qwen': {
            ...
            // rewrites below) instead of throwing on `_b['copilot-instructions.md']`.
            const _b = _hostBehaviors(runtime).brandingRewrites;
            if (_b) {
                content = content.replace(/CLAUDE\.md/g, _b['copilot-instructions.md']);
            ...
            if (_b) {
                content = content.replace(/\.claude\//g, _b['.github/']);
            }
            content = content.replace(/\.\/\.claude\//g, `./${dirName}/`);
...
/**
 * Apply the 4 base `.github/` path-prefix rewrites to a single agent content
 * string. Mirrors the inline agent loop in bin/install.js lines 9330-9340:
 *   ~/\.github/ → pathPrefix
 *   $HOME/\.github/ → pathPrefix
