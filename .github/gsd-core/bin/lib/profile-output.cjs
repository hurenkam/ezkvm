...
// copilot-instructions.md fallback / placeholder text — runtime-aware so emitted slash
...
function buildClaudeMdFallbacks(runtime) {
    return {
        ...
        skills: 'No project skills found. Add skills to any of: `.github/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/`, or `.codex/skills/` with a `SKILL.md` index file.',
    };
}
// Directories where project skills may live (checked in order)
const SKILL_SEARCH_DIRS = ['.github/skills', '.agents/skills', '.cursor/skills', '.github/skills', '.codex/skills'];
...
function cmdWriteProfile(cwd, options, raw) {
    ...
    if (!outputPath) {
        ...
        // the agent home, so a Codex run wrote the profile under the the agent config dir
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
    ...
    let targetPath;
    if (options.global) {
        targetPath = node_path_1.default.join(node_os_1.default.homedir(), '.claude', 'copilot-instructions.md');
    }
    else if (options.output) {
        targetPath = node_path_1.default.isAbsolute(options.output) ? options.output : node_path_1.default.join(cwd, options.output);
    }
    else {
        // Read claude_md_path from config; #1098 default is ./.github/copilot-instructions.md
        ...
        let configClaudeMdPath = './.github/copilot-instructions.md';
        try {
            const config = loadConfig(cwd);
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    // `.github/copilot-instructions.md` (a valid auto-loaded memory location) rather than a
    // repo-root `copilot-instructions.md`, so generated GSD content does not land next to — or
    // pollute — a hand-crafted repo-root copilot-instructions.md. An explicit `claude_md_path`
    ...
    try {
        ...
        // copilot-instructions.md; copilot now resolves to .github/copilot-instructions.md, and
        ...
        // rationale: a Codex/AGENTS-native project must never write to copilot-instructions.md
        // even if a prior the agent setup left a `claude_md_path` behind). For the
    ...
    if (existingContent === null) {
        ...
        // section markers — it is a hand-crafted copilot-instructions.md/AGENTS.md, not a
