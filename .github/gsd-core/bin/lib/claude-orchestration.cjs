"use strict";
/**
 * the agent Orchestration Capability — Workflow-tool backend detection + emitter
 *
 ...
 *   2. runtime is the agent (the only runtime that exposes the Workflow tool)
...
function detectWorkflowBackend(input) {
    ...
    // 2. only the agent can host the Workflow tool.
    ...
    //    gated at step 2; `background:true` alone is true on several non-the agent hosts,
