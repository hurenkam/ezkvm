...
function buildMsgDiverged(headSha, forkRef, forkSha) {
    return `⚠ Worktree base mismatch: HEAD (${shortSha(headSha)}) differs from ${forkRef} (${shortSha(forkSha)}). Running this phase sequentially on the main working tree. To keep parallel worktrees, set worktree.baseRef:"head" in .github/settings.local.json (or run: gsd-tools worktree set-baseref). See #683.`;
}
const MSG_UNKNOWN = `⚠ Cannot determine the worktree fork base (origin/HEAD unresolved). Running this phase sequentially on the main working tree to avoid a base mismatch. To keep parallel worktrees, set worktree.baseRef:"head" in .github/settings.local.json (or run: gsd-tools worktree set-baseref). See #683.`;
// ─── Exports ──────────────────────────────────────────────────────────────────
/**
...
/**
 * CLI command: write worktree.baseRef = 'head' into <cwd>/.github/settings.local.json.
 *
