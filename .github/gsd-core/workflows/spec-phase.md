...
ones). Reference: @.github/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / .github.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "$_GSD_RT/.github/bin/lib/edge-probe.cjs" \
  ".github/gsd-core/bin/lib/edge-probe.cjs" \
  ".github/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @.github/gsd-core/references/prohibition-probe.md — the
...
Use the SPEC.md template from @.github/gsd-core/templates/spec.md.
