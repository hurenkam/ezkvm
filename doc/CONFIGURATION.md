...
- On AppArmor-enabled hosts with the distro `swtpm` profile (`/etc/apparmor.d/usr.bin.swtpm`), preflight also validates that TPM socket paths and local TPM backend paths are allowed by policy. Common allowed socket paths are `/run/libvirt/qemu/swtpm/<name>.sock` and `/run/swtpm/sock`; paths such as `/var/run/ezkvm/*.socket` or backend device paths under `/dev` are typically denied unless a local AppArmor override is added. The Debian package ships `/etc/apparmor.d/local/usr.bin.swtpm` to allow ezkvm runtime sockets/pid files under `/run|/var/run/ezkvm`, logs under `/var/log/ezkvm`, and Proxmox-style TPM backend device paths under `/dev/vm1/vm-*-tpmstate` and `/dev/dm-*`.
