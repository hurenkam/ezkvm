...
This document defines architecture best practices for this repository. It complements `doc/dev/workflow/coding-guidelines.md` by describing how to organize code and components at the system level.
...
Major architecture decisions are recorded in `doc/dev/architecture/decisions/` for visibility and future reference.

### Current ADRs

- [ADR-0001: Base Selection](decisions/ADR-0001-base-selection.md) - Why current codebase was chosen as foundation for incremental convergence
- [ADR-0002: Import Normalization Contract](decisions/ADR-0002-import-normalization-contract.md) - How external configs (Proxmox) normalize to canonical schema
- [ADR-0003: Hooks Policy](decisions/ADR-0003-hooks-policy.md) - VM lifecycle hooks design (pre/post start/stop)
- [ADR-0004: Trait Seam Policy](decisions/ADR-0004-trait-seam-policy.md) - Where and how trait-based extensibility is allowed
- [ADR-0005: Q35 Topology Contract](decisions/ADR-0005-q35-topology-contract.md) - Why ezkvm preserves Proxmox-aligned Q35 PCIe/PCI topology and slot stability for imports

Concrete seam definitions and examples live in `doc/dev/architecture/extensibility-seams.md`.

### Adding New ADRs

When proposing an architecture decision:
1. Create a new file in `doc/dev/architecture/decisions/ADR-NNNN-{title}.md`
